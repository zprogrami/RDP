...
uses
  ...
  Controls, Forms, Dialogs, OleCtrls, OleServer, MSTSCLib_TLB, Registry;

type
  TRDPForm = class(TForm)
    RDPmstsc: TMsRdpClient9NotSafeForScripting;
...
implementation
{$R *.dfm}
procedure TRDPForm.FormCreate(Sender: TObject);
var
  Reg: TRegistry;
begin
  RDPmstsc.DisconnectedText := 'Disconnected.';
  RDPmstsc.ConnectingText := 'Connecting...';
  RDPmstsc.ConnectedStatusText := 'Connected.';
  RDPmstsc.UserName := '';
  RDPmstsc.Server := '127.0.0.2';
  ...
  if Reg.OpenKeyReadOnly('\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp') then begin
    try
      RDPmstsc.AdvancedSettings2.RDPPort := Reg.ReadInteger('PortNumber');
    except
    end;
    Reg.CloseKey;
  end;
...
procedure TRDPForm.RDPDisconnected(ASender: TObject; discReason: Integer);
var
  ErrStr: String;
  Reg: TRegistry;
begin
